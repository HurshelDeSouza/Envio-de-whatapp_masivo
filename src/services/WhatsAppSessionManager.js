...
class WhatsAppSessionManager {
  ...
  async getSession(phone) {
    ...
    if (this.sessions.has(cleanPhone)) {
      ...
      console.log(
        `📱 Sesión existente encontrada para ${cleanPhone}, estado: ${session.status}`
      );

      // Si la sesión ya está lista, emitir evento inmediatamente
      if (session.status === "ready") {
        console.log(`✅ Sesión ya está lista, emitiendo evento whatsapp-ready`);
        this.io.emit("whatsapp-ready", { phone: cleanPhone });
      }
  ...
  setupClientEvents(phone, client) {
    ...
    client.on("disconnected", (reason) => {
      ...
      // No intentar eliminar archivos si es LOGOUT para evitar errores EBUSY
      if (reason === "LOGOUT") {
        console.log(
          `ℹ️ ${phone} - Logout detectado, manteniendo archivos de sesión`
        );
      }
    });
  }
